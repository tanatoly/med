...
{
	public static final G711A INSTANCE = new G711A();
	...
	{
//		int headerLength = input.limit() - ONE_PACKAGE;
		output.limit(320);
//
//
...
//		}
		...
		int j = output.position();
		int currentIndex = 0;

		for (; i < input.limit(); i++)
		{
			currentIndex = input.get(i) & 0xff;
			byte lcurrent = aLawDecompressTable_low[currentIndex];
			byte hcurrent = aLawDecompressTable_high[currentIndex];
			output.put(j++,lcurrent);
			output.put(j++,hcurrent);
		}
	...
	{
//		int headerLength 	= input.limit() - (ONE_PACKAGE * 2);
//		output.limit(headerLength + ONE_PACKAGE);
//
//		for (int i = 0; i < headerLength; i++)
//		{
//			output.put(input.get());
//		}
		...
		for (; i < output.limit(); i++,j++)
		{
			byte current = aLawCompressTable[input.get(j+1) & 0xff][input.get(j++) & 0xff];
			output.put(i,current);
		}
		output.position(output.limit());
	}

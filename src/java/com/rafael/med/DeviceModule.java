...
public final class DeviceModule extends GridPane
{
	...
	public DeviceModule()
	{
		...
		{	
			...
			GridPane.setConstraints(rows[i].name, 				0, i + 1, 3, 1, HPos.LEFT, VPos.CENTER);
			GridPane.setConstraints(rows[i].value, 				3, i + 1, 2, 1, HPos.LEFT, VPos.CENTER);
			GridPane.setConstraints(rows[i].units, 				5, i + 1, 2, 1, HPos.LEFT, VPos.CENTER);
			GridPane.setConstraints(rows[i].defaultValue, 		7, i + 1, 1, 1, HPos.LEFT, VPos.CENTER);
			GridPane.setConstraints(rows[i].range, 				8, i + 1, 2, 1, HPos.LEFT, VPos.CENTER);
